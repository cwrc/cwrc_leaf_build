...
function update_leaf_version() {
    ...
    python3 ./docker/drupal/scripts/leaf_version_bake.py \
...
function update_core_extension() {
    ...
    python3 ./docker/drupal/scripts/merge_cwrc_customizations_drupal_core_extensions_yaml.py \
...
function update_composer_json_lock() {
    ...
    composer require -d ./docker/drupal/rootfs/var/www/drupal \
        --no-scripts --no-plugins --no-install --no-audit --no-interaction \
        'drupal/getjwtonlogin:^2.0' 'mjordan/islandora_bagger_integration'
}
