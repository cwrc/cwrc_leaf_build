...
merge_composer_json() {
    ...
    local composer_json_path=${1:-/var/www/drupal/composer.json}
    ...
    if [ $? == 0 ] ; then
        echo "URL Found - no action required on composer.json";
    else
        echo "URL Not Found - action required to add CWRC customisations to composer.json";
        jq -e '.repositories += [{"type": "git", "url": "https://github.com/cwrc/islandora_bagger_integration.git", "no-api": true}]' ${composer_json_path} > ${composer_json_path}.new && \
        mv ${composer_json_path} ${composer_json_path}.bak && \
        mv ${composer_json_path}.new ${composer_json_path}
    fi
}

merge_composer_json ${COMPOSER_JSON_PATH}